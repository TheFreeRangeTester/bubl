...
alter table public.users
add column if not exists last_feed_seen_week_id text;
...
declare
  ...
  viewer_last_feed_seen_at timestamptz;
  viewer_last_feed_seen_week_id text;
begin
  select u.last_feed_seen_at, u.last_feed_seen_week_id
    into viewer_last_feed_seen_at, viewer_last_feed_seen_week_id
  from public.users u
      ...
      'new_related_count',
        coalesce(
          (
            select count(*)
            from selected_candidates
            where viewer_last_feed_seen_week_id is distinct from current_week_id
               or viewer_last_feed_seen_at is null
               or selected_candidates.created_at > viewer_last_feed_seen_at
          ),
          0
        )
    )
